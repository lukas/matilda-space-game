#!/usr/bin/env python3
"""
Regenerate audio files that still have character names in the spoken text
"""
import json
import requests
import hashlib
...
def main():
    ...
    # Find files that have character names in clean_text
    files_to_regenerate = []
    ...
    print(f"\n🔍 Found {len(files_to_regenerate)} files that need character name removal")
    
    # Show what we're fixing
    for i, file_entry in enumerate(files_to_regenerate[:10]):
        print(f"  {i+1}. [{file_entry['character']}] {file_entry['clean_text'][:50]}...")
